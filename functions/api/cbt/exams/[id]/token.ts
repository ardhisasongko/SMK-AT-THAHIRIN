...
import { canManageCbtExam, hashCbtToken, isCbtStaff } from '../../../../_lib/cbt';
...
export const onRequestPost: PagesFunction<Env, any, AuthData> = async ({ env, data, params }) => {
  ...
  const exam = await env.DB.prepare('SELECT teacher_user_id,teacher_name FROM cbt_exams WHERE id=?').bind(String(params.id)).first<any>();
  if (!exam) return jsonResponse({ success: false, error: 'Ujian tidak ditemukan.' }, 404);
  if (!canManageCbtExam(data.user, exam)) return jsonResponse({ success: false, error: 'Guru hanya dapat merotasi token ujian miliknya.' }, 403);
