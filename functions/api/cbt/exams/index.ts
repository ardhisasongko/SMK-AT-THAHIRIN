...
function examTypeValue(body: any): 'ujian' | 'latihan' {
  return body.examType === 'ujian' ? 'ujian' : 'latihan';
}

function minSubmitValue(body: any): number | null {
  if (body.examType !== 'ujian') return null;
  return body.minSubmitMinutes ?? Math.round(Number(body.durationMinutes) * 0.8);
}
...
export const onRequestGet: PagesFunction<Env, any, AuthData> = async ({ env, data }) => {
  ...
  for (const row of results as any[]) {
    ...
    exams.push({
      ...
      examType: String(row.exam_type || 'latihan'), minSubmitMinutes: row.min_submit_minutes != null ? Number(row.min_submit_minutes) : undefined,
...
export const onRequestPost: PagesFunction<Env, any, AuthData> = async ({ env, data, request }) => {
  ...
  const statements = [
    env.DB.prepare(`INSERT INTO cbt_exams (id,title,subject,class_target,duration_minutes,token_hash,teacher_user_id,teacher_name,start_date,end_date,open_time,close_time,status,exam_type,min_submit_minutes)
      VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`).bind(id, body.title.trim(), body.subject.trim(), String(body.classTarget).slice(0, 100), body.durationMinutes, await hashCbtToken(body.token), data.user.id, data.user.name, startDate, endDate, body.openTime || null, body.closeTime || null, status, examTypeValue(body), minSubmitValue(body)),
...
export const onRequestPatch: PagesFunction<Env, any, AuthData> = async ({ env, data, request }) => {
  ...
  const statements = [
    env.DB.prepare(`UPDATE cbt_exams SET title=?,subject=?,class_target=?,duration_minutes=?,start_date=?,end_date=?,open_time=?,close_time=?,status=?,exam_type=?,min_submit_minutes=?,updated_at=datetime('now') WHERE id=?`).bind(body.title.trim(), body.subject.trim(), body.classTarget, body.durationMinutes, body.startDate, body.endDate, body.openTime || null, body.closeTime || null, storedStatus, examTypeValue(body), minSubmitValue(body), exam.id),
  ...
  if (body.token) statements[0] = env.DB.prepare(`UPDATE cbt_exams SET title=?,subject=?,class_target=?,duration_minutes=?,start_date=?,end_date=?,open_time=?,close_time=?,status=?,token_hash=?,exam_type=?,min_submit_minutes=?,updated_at=datetime('now') WHERE id=?`).bind(body.title.trim(), body.subject.trim(), body.classTarget, body.durationMinutes, body.startDate, body.endDate, body.openTime || null, body.closeTime || null, storedStatus, await hashCbtToken(body.token), examTypeValue(body), minSubmitValue(body), exam.id);
  try {
    await env.DB.batch(statements);
  } catch (error) {
