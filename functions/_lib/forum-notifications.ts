...
export async function ensureLegacyForumMigrated(db: D1Database): Promise<void> {
  ...
  for (const topic of topics) {
    ...
    const statements = [
      db.prepare(`INSERT OR IGNORE INTO forum_topics (id,title,category_type,category_name,author_user_id,author_name,author_role,author_avatar,content,tags_json,attachments_json,legacy_like_count,view_count,is_pinned,is_resolved,created_at)
        VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`).bind(String(topic.id), String(topic.title).slice(0,200), topic.categoryType === 'kelas' ? 'kelas' : 'mapel', String(topic.categoryName || '').slice(0,100), String(topic.authorId || 'legacy'), String(topic.authorName || 'Pengguna'), String(topic.authorRole || 'siswa'), String(topic.authorAvatar || ''), String(topic.content || '').slice(0,10000), JSON.stringify(topic.tags || []), JSON.stringify(topic.attachments || []), Math.max(0, Number(topic.likes || 0) - likedBy.length), Math.max(0, Number(topic.views || 0)), topic.isPinned ? 1 : 0, topic.isResolved ? 1 : 0, new Date().toISOString()),
