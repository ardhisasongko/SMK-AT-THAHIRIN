...
async function cycle(){if(!ready)return;const claimed=await api({action:'claim'});active=claimed.active===true;const messages=claimed.data||[];if(!messages.length){emptyRuns++;return;}emptyRuns=0;const results=[];for(const m of messages){try{await client.sendMessage(`${m.recipient_phone}@c.us`,m.message_text);results.push({id:m.id,success:true});}catch(e){results.push({id:m.id,success:false,error:String(e?.message||e).slice(0,180)});}await sleep(cfg.minDelay+Math.floor(Math.random()*(cfg.maxDelay-cfg.minDelay+1)));}await api({action:'complete',claimToken:claimed.claimToken,results});console.log(`Batch selesai: ${results.filter(r=>r.success).length}/${results.length} terkirim.`);}
async function loop(){while(true){try{await cycle();}catch(e){console.error(new Date().toISOString(),e.message);}const interval=active?(emptyRuns>=3?Math.min(cfg.idleMs,300000):cfg.activeMs):cfg.idleMs;await sleep(interval);}}
